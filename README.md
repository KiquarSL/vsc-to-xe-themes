# Themes for Xed-Editor

This is list of themes for [Xed-Editor](https://github.com/Xed-Editor/Xed-Editor)

See themes preview in [themes/README.md](themes/README.md)

## How to use

1. Download the `*.json` theme from `themes/` or clone this repository: `clone https://github.com/KiquarSL/xe-themes`
4. Go to **Xed-Editor → Settings → Themes → Add theme**
5. Select your imported theme to apply the theme

